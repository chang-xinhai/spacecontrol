...
def load_template_mesh(file_path: str):
    ...
    if isinstance(loaded, trimesh.Scene):
        ...
        for geom in loaded.geometry.values():
            if isinstance(geom, trimesh.Trimesh) and len(geom.vertices) > 0 and len(geom.faces) > 0:
                meshes.append(geom)
        if not meshes:
            raise ValueError(f"No triangle meshes found in scene: {file_path}")
    ...
    if len(vertices) == 0 or len(faces) == 0:
        raise ValueError(f"Template mesh is empty: {file_path}")
...
def canonicalize_mesh(vertices: np.ndarray):
    ...
    if extent <= 0:
        raise ValueError("Template mesh has degenerate bounds; cannot normalize")
...
def main():
    parser = argparse.ArgumentParser(description="Headless local SpaceControl generation from a template mesh and save.")
    parser.add_argument("--template", type=str, default="data/GSO_example/BEDROOM_NEO/model_canonical.ply")
    ...
    parser.add_argument("--t0", type=float, default=12.0)
    ...
    print("[stage] load template mesh")
    template_vertices, template_faces = load_template_mesh(args.template)
    stage_file(run_dir, "template_mesh_loaded")

    print("[stage] canonicalize template mesh")
    canonical_vertices, center, scale = canonicalize_mesh(template_vertices)
    spatial_control_mesh_path = run_dir / "spatial_control_mesh.ply"
    trimesh.Trimesh(vertices=canonical_vertices, faces=template_faces, process=False).export(str(spatial_control_mesh_path))
